...
class GlobalPlanBuilder:
    ...
    def build(self, sku_pool: List[Dict[str, Any]]) -> Optional[List[Dict[str, Any]]]:
        ...
        zones = {
            'rear': [k for k in info if _classify(by_sku[k]) == 'rear'],
            'middle': [k for k in info if _classify(by_sku[k]) == 'middle'],
            'door': [k for k in info if _classify(by_sku[k]) == 'door'],
            'flex': [k for k in info if _classify(by_sku[k]) == 'flex'],
        }
