...
class WallOptimizationEngine:
    ...
    def optimize(self,wall_plan,remaining_cargo,container,door_wall,reserved_inventory=None):
        ...
        expansion=self.expansion.expand(optimized,remaining_cargo,container,min(p.x for p in door_wall.placements),reserved_inventory)
        ...
        chain_strength=sum(e.strength for e in chain.connections)/max(len(chain.connections),1)
